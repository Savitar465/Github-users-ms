...
public class KeycloakRolesServiceImpl implements KeycloakRolesService {
    ...
    @Override
    public List<RoleRepresentation> getRealmRoles(Integer max, Integer first) {
        return keycloak.realm(realm).roles().list(first,max);
    }

    @Override
    public List<RoleRepresentation> getClientRoles(String clientUuid, Integer max, Integer first) {
        return keycloak.realm(realm).clients().get(clientUuid).roles().list(first,max);
    }

    @Override
    public RoleRepresentation getRole(String rolId) {
